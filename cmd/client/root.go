...
store logins, passwords, binary data, and other private information.`,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg, err := config.NewClientConfig(config.WithGlobalViper())
			if err != nil {
	...
	rootCmd.PersistentFlags().
		StringP("server-url", "s", "", "server base URL")
	rootCmd.PersistentFlags().
		Bool("insecure-skip-verify", false, "skip TLS certificate verification")
